...
local lastCheckedPhase = 200;-- default
local npcList = {};
local forgeList = {};
local filteredList = {};
...
function findValue(array,valueName,value)
   local valueExists = false;
   for _, field in ipairs(array) do
      if field[valueName] == value then
         valueExists = true;
      end
   end
   return valueExists;
...
server.receive("PNPCS", function(message, channel, sender)
   ...
   for _, record in pairs(records) do
      ...
      if id and field and guid then
         ...
         local exists = findValue(forgeList,"id",id);
         if not exists then
...
scroll:SetScript("OnUpdate", function(self)
   ...
   local max = listState == "forge" and #forgeList or listState == "filter" and #filteredList or #npcList[listState];
   ...
   for i=1, #buttons do
      ...
      if localNpcList then
         if localNpcList == self.selected then 
            ...
            if type(npcList[scroll.selected.id]) ~= "table" then
               next:Disable();
            elseif listState == "forge" or listState == "filter" then
               next:Enable();
            end
         else 
         ...
         button.right:SetText(localNpcList.guid or npcList[localNpcList.id] ~= null and "Press >" or "Loading guids...");
         button:Show();
      else
         button:Hide();
      end
...
local function CreateSet(text, tooltip, name)
	...
	self:SetScript("OnEvent", function(self, event, key, state)
...
previous:SetDisabledTexture("Interface/Buttons/UI-SpellbookIcon-PrevPage-Disabled")
...
search:SetScript("OnEnterPressed", function(self)
   ...
   if filterText == "" then
      ...
      for _, npc in ipairs(forgeList) do
         if npc.id:lower():find(filterText:lower()) or npc.name:lower():find(filterText:lower()) then
